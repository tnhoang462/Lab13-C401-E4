...
- [TASKS_COMPLETED]:
  - Xác lập Baseline hiệu năng hệ thống (Latency P50 ~160ms).
  - Thực hiện Chaos Engineering: Giả lập thành công 3 sự cố (`rag_slow`, `cost_spike`, `tool_fail`) để kiểm thử hệ thống giám sát.
  - Phân tích Stress Test: Đo lường tác động của tải cao (concurrency=5) lên độ trễ hệ thống (tăng vọt lên 13.3s).
  - Enrich Test Data: Bổ sung dữ liệu PII và Edge cases vào `sample_queries.jsonl` để kiểm tra khả năng Sanitization.
